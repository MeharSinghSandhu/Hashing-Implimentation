...
static HashAlgorithm lookupNamedHashStrategy(const char *name)
{
	...
	else if (strncmp(name, "firstchar", 9) == 0) {
        // New strategy: hash by first character
        return hashByFirstChar;
    }
 ...
 */
    ...
    // Calculate the initial index using the primary hash algorithm
...
void *aaLookup(AssociativeArray *aarray, AAKeyType key, size_t keylen) {
    ...
    while (attempt < aarray->size) {
        ...
        // Increment search cost for each slot probed
        aarray->searchCost++;
    ...
    return NULL; // Key not found after full table scan
}

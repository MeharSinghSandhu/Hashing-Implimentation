 ...
 *  @param  size boundary for range of allow
 able return values
...
HashIndex hashByFirstChar(AAKeyType key, size_t keyLength, HashIndex size) {
    if (keyLength == 0) {
        return 0; 
    }
    return key[0] % size;
...
HashIndex linearProbe(AssociativeArray *hashTable, AAKeyType key, size_t keyLength, int index, int invalidEndsSearch, int *cost) {
    ...
    while (1) {
        HashIndex currentIndex = (originalIndex + attempt) % hashTable->size;
	...
	)
{
    ...
    while (attempt < hashTable->size) {
        currentIndex = (startIndex + attempt * attempt) % hashTable->size;
        KeyDataPair currentPair = hashTable->table[currentIndex];
        if (currentPair.validity == HASH_EMPTY || (invalidEndsSearch && currentPair.validity == HASH_DELETED)) {
            if (cost) *cost += attempt;
            return currentIndex;
...
// 	 */
    ...
    int attempt = 0;
    while (attempt < hashTable->size) {
        int currentIndex = ((startIndex) + (attempt * stepSize)) % (hashTable->size);
        KeyDataPair currentPair = hashTable->table[currentIndex];
        if (currentPair.validity == HASH_EMPTY || (invalidEndsSearch && currentPair.validity == HASH_DELETED)) {
            if (cost) *cost += attempt;
            return currentIndex;
